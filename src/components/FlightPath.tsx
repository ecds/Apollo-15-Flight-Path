...
const FlightPath = ({ className, scrollProgress }: Props) => {
  ...
  useEffect(() => {
    ...
    if (floor === 4 && scrollProgress >= 4.5) {
      setCurrentSegment(4.5);
    } else {
        ...
        >
          <g id="ascent-path">
            ...
            <PathSegment
              ...
              scrollProgress={scrollProgress}
              setPoint={setPoint}
              shape="M466.8,66.2c-93.9,0-178.5,68.3-192,189.6-7,62.3-9,167.5,6,273.7,24.6,173.6,105.9,393.1,185,583.7,55.6,134,117.5,255,121.4,341.3,0,2.9.2,5.8,0,8.6"
            />
            <PathSegment
              id="launch"
              segment={1}
              className={
                scene === "launch" || scene == "ascent"
                  ? "opacity-100"
                  : "opacity-20"
              }
              scrollProgress={scrollProgress}
              setPoint={setPoint}
              shape="M543,236.3c-71.5,32-138.6,123.9-90.9,176.2,37.7,41.2,109,6,109,6,54.3-31.3,91.7-87.9,94.7-154.4,4.9-107.9-81-197.4-189-197.9"
            />
